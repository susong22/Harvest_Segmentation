...
def train_main():
    ...
    # 라벨에 따라 n_classes_without_void 동적으로 계산
    all_labels = torch.cat([sample['label'].view(-1) for sample in valid_loader], dim=0)
    n_classes_without_void = int(all_labels.max().item() + 1)
    print(f"Number of classes: {n_classes_without_void}")
    ...
    for sample in train_loader:
        print(f"Train Lable uniuqe values: {torch.unique(sample['label'])}")
        break

    for sample in valid_loader:
        print(f"Valid Lable uniuqe values: {torch.unique(sample['label'])}")
        break
...
######################loss function 바꿈  ###############3
    ...
    # NumPy 배열을 PyTorch Tensor로 변환 (device와 dtype 설정)
    pos_weight = torch.tensor(class_weighting, dtype=torch.float32, device=device)
    print(f"Class weighting: {class_weighting}")
    
    # BCEWithLogitsLoss 초기화
    loss_function_train = torch.nn.BCEWithLogitsLoss(pos_weight=pos_weight)
    ...
    # Validation용 loss function 초기화
    loss_function_valid = torch.nn.BCEWithLogitsLoss(
        pos_weight=pos_weight,
        reduction='mean'
    )

    loss_function_valid_unweighted = torch.nn.BCEWithLogitsLoss()
 ...
 ######################loss function 바꿈  ###############3
    ...
    confusion_matrices = {
        camera: ConfusionMatrixTensorflow(n_classes_without_void)
        for camera in cameras
    }
    confusion_matrices['all'] = ConfusionMatrixTensorflow(n_classes_without_void)
...
#         print(f"Debugging Batch {i}")
#         print(f" - Label Min: {sample['label'].min()}, Max: {sample['label'].max()}, Unique: {torch.unique(sample['label'])}")
...
#         # print(f" - Image Min: {sample['image'].min()}, Max: {sample['image'].max()}, Mean: {sample['image'].mean().item()}")
#         # print(f" - Depth Min: {sample['depth'].min()}, Max: {sample['depth'].max()}, Mean: {sample['depth'].mean().item()}")
#         # print(f" - Label Min: {sample['label'].min()}, Max: {sample['label'].max()}, Mean: {sample['label'].mean().item()}")
...
#         for j, pred in enumerate(pred_scales):
#             print(f"Pred Scale {j} - Min: {pred.min()}, Max: {pred.max()}, Mean: {pred.mean().item()}")

#         # loss computation
#         losses = loss_function_train(pred_scales, target_scales)
#         loss_segmentation = sum(losses)
        
#         print(f" - Loss Segmentation: {loss_segmentation.item()}")
...
#         #         print(f" - Grad {name} - Min: {param.grad.min()}, Max: {param.grad.max()}, Mean: {param.grad.mean().item()}")
...
def train_one_epoch(model, train_loader, device, optimizer, loss_function_train,
    ...
    for i, sample in enumerate(train_loader):
        ...
        # Debugging for predictions
        for j, pred in enumerate(pred_scales):
            print(f"Pred Scale {j} - Min: {pred.min()}, Max: {pred.max()}, Mean: {pred.mean().item()}")

        # Target scales adjustment (One-Hot Encoding to match prediction size)
        target_scales = [F.one_hot(t.squeeze(1).long(), num_classes=pred_scales[0].shape[1])
                            .permute(0, 3, 1, 2).float()  # Reshape to (N, C, H, W)
                        for t in [sample['label'].to(device)]]
        ...
        # Loss computation
        losses = []
        for j, (pred, target) in enumerate(zip(pred_scales, target_scales)):
            loss = loss_function_train(pred, target)
            losses.append(loss)
    ...
    # Fill logs for CSV log file and web logger
    logs = dict()
    logs['time_training'] = time.time() - training_start_time
    logs['loss_train_total'] = np.mean(total_loss_list)
    losses_train = np.mean(losses_list, axis=0)
    ...
    #     logs['loss_train_down_{}'.format(rate)] = losses_train[i + 1]
    for i, rate in enumerate(label_downsampling_rates):
        if i + 1 < len(losses_train):  # Ensure index is within bounds
            logs['loss_train_down_{}'.format(rate)] = losses_train[i + 1]
        else:
            print(f"Warning: Skipping rate {rate} due to index out of bounds.")
...
def validate(model, valid_loader, device, cameras, confusion_matrices,
    ...
    #손실함수 변경, reset_loss() 함수 삭제 후 누적 손실값 초기화 코드 추가
    ...
    for camera in cameras:
        with valid_loader.dataset.filter_camera(camera):
            ...
            for i, sample in enumerate(valid_loader):
                ...
                with torch.no_grad():
                    ...
                    if modality == 'rgbd':
                        ...
                        print(f"Validation Prediction Min - Min: {prediction.min()}, Max: {prediction.max()}, Mean: {prediction.mean().item()}")
                    ...
                    print(f"Prediction range: Min: {prediction.min()}, Max: {prediction.max()}, Mean: {prediction.mean().item()}")
                    print(f"Label range: Min={sample['label'].min()}, Max={sample['label'].max()}, Mean={sample['label'].float().mean().item()}")
                    
                    # One-Hot Encoding for target
                    target = F.one_hot(sample['label'].squeeze(1).long(), num_classes=prediction.shape[1])
                    target = target.permute(0, 3, 1, 2).to(device, dtype=torch.float)  # Reshape to (N, C, H, W)

                    # 손실 계산
                    validation_loss = loss_function_valid(prediction, target)

                    
                    total_validation_loss += validation_loss.item()
                    
                    if loss_function_valid_unweighted is not None:
                        # One-Hot Encoding for unweighted loss calculation
                        target_unweighted = F.one_hot(sample['label'].squeeze(1).long(), num_classes=prediction.shape[1])
                        target_unweighted = target_unweighted.permute(0, 3, 1, 2).to(device, dtype=torch.float)  # (N, C, H, W)

                        unweighted_loss = loss_function_valid_unweighted(prediction, target_unweighted)
                    ...
                    # print(f"Validation Loss: {loss_function_valid.compute_whole_loss()}")
                    ...
                    # if loss_function_valid_unweighted is not None:
                    #     unweighted_loss = loss_function_valid_unweighted(
                    #         prediction, sample['label'].unsqueeze(1).to(device, dtype=torch.float)
                    #     )
                    #     total_unweighted_loss += unweighted_loss.item()
                    ...
                    if loss_function_valid_unweighted is not None:
                        ...
                        print(f"Total Unweighted Validation Loss: {total_unweighted_loss}")
                    ...
                    #print(f"Label unique values: {np.unique(label.cpu())}, Prediction unique values: {np.unique(prediction.cpu())}")
            ...
            print(f"Overall Confusion Matrix : {confusion_matrices[camera].overall_confusion_matrix}")
            cm_time += time.time() - cm_start_time
            print(f"Total Validation Loss: {total_validation_loss}")
    ...
    logs[f'loss_{valid_split}'] = total_validation_loss / len(valid_loader.dataset)

    if loss_function_valid_unweighted is not None:
        logs[f'loss_{valid_split}_unweighted'] = total_unweighted_loss / len(valid_loader.dataset)
    ...
    # Epoch 결과 출력
    print(f"Epoch {epoch + 1}: Validation Loss={total_validation_loss / len(valid_loader.dataset):.6f}, "
        f"Unweighted Loss={total_unweighted_loss / len(valid_loader.dataset):.6f}, "
        f"mIoU={miou['all']:.6f}")
