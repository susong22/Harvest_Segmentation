...
def prepare_data(args, ckpt_dir=None, with_input_orig=False, split=None):
    ...
    #     if 0 not in np.unique(label):
    #         label[0, 0] = 0  # 임의 위치에 0 추가
    #         train_data[idx]['label'] = label
    #     print(f"Preprocessed Label Unique Values (Index {idx}): {np.unique(label)}")
    #       # 한 샘플만 확인
